...
export const CommunitySection = (): JSX.Element => {
  // Background grid data for the decorative elements
  const gridRowsData = [
  ...
  return (
    <section className="flex flex-col items-center w-full flex-[0_0_auto] bg-[#000d19] rounded-[4px_4px_0px_0px] relative self-stretch">
      {/* Main content area with pixel art and message */}
      <div className="items-center gap-24 px-0 py-24 self-stretch w-full flex-[0_0_auto] bg-[#000d19] overflow-hidden border-t-[24px] [border-top-style:solid] border-r-[24px] [border-right-style:solid] border-l-[24px] [border-left-style:solid] border-[#19223a] flex flex-col relative">
        <div className="flex flex-col items-center justify-center gap-8 px-24 py-0 relative self-stretch w-full flex-[0_0_auto]">
          ...
          <h2 className="relative w-[902px] font-display-h2v1 font-[number:var(--display-h2v1-font-weight)] text-white text-[length:var(--display-h2v1-font-size)] text-center tracking-[var(--display-h2v1-letter-spacing)] leading-[var(--display-h2v1-line-height)] [font-style:var(--display-h2v1-font-style)]">
        ...
        <div className="absolute w-[1858px] h-[782px] top-[-81px] left-[-209px]">
          {gridRowsData.map((row, rowIndex) => (
            <div
              key={`grid-row-${rowIndex}`}
              className={`inline-flex items-center gap-3 absolute ${row.top} ${row.left}`}
            >
              {Array.from({ length: row.count }).map((_, cellIndex) => (
                <Card
